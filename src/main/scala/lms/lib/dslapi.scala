...
abstract class DslDriverC[A:Manifest,B:Manifest] extends DslSnippet[A,B] with DslExp { q =>
  ...
  def eval(a:A): Unit = { // TBD: should read result of type B?
    ...
    (s"cc -std=c99 -O3 /tmp/snippet.c -o /tmp/snippet":ProcessBuilder).lineStream.foreach(Console.println)
    (s"/tmp/snippet $a":ProcessBuilder).lineStream.foreach(Console.println)
  }
}
