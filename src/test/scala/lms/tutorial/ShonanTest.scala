/*
The task is to implement a sparse matrix vector product, where the matrix is
statically known but the vector is not. This situation comes up in hidden markov
models (HMM), where a single transition matrix is multiplied by many different
observation vectors. Therefore, it pays off to generate code that is specialized
to a particular matrix.

For further reference, see:

- Shonan Challenge for Generative Programming
  ([PDF](https://www.cs.rutgers.edu/~ccshan/metafx/pepm2013.pdf)) ([official code repository](https://github.com/StagedHPC/shonan-challenge))
...
class ShonanTest extends TutorialFunSuite {
  ...
  test("shonan-hmm1a") {
    ...
    // The result is a concrete vector
  ...
  // The condition below is fully static, so it does not show up in
